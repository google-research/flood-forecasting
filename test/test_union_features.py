...
def base_dataset(sample_basins, sample_dates, sample_lead_times):
    ...
    ds['feature_2d_hindcast'].loc[
        {'basin': 'basin_A', 'date': '2000-01-02'}
    ] = np.nan
    ds['feature_3d_forecast'].loc[
        {
            'basin': 'basin_B',
            'date': '2000-01-03',
...
def test_expand_lead_times_basic_expansion(
    ...
    expected_lead_times = np.array(
        [np.timedelta64(i, 'D') for i in range(1, 3)]
    )
...
def test_expand_lead_times_raises_error_if_lead_time_exists(
    ...
    da_with_lead_time = xr.DataArray(
        np.random.rand(
            len(sample_basins), len(sample_dates), len(sample_lead_times)
        ),
        coords={
            'basin': sample_basins,
            'date': sample_dates,
    ...
    with pytest.raises(
        ValueError,
        match='Trying to expand a dataarray that already has a lead time.',
    ):
...
def test_union_features_with_same_dimensions():
    ...
    da_values = xr.DataArray(
        [[1, 2], [3, 4]], coords={'x': [0, 1], 'y': [0, 1]}, dims=['x', 'y']
    )
    xr.testing.assert_equal(
        _union_features_with_same_dimensions(da_nan, da_values), da_values
    )
    xr.testing.assert_equal(
        _union_features_with_same_dimensions(da_values, da_nan), da_values
    )
...
def test_union_lead_time_feature_with_non_lead_time_feature(
    ...
    result = _union_lead_time_feature_with_non_lead_time_feature(
        feature_da, mask_feature_da
    )
    xr.testing.assert_equal(result, expected)


def test_union_non_lead_time_feature_with_lead_time_feature(
    sample_basins, sample_dates, sample_lead_times
):
    feature_da = xr.DataArray(
        [[10, np.nan, 20]],
    ...
    result = _union_non_lead_time_feature_with_lead_time_feature(
        feature_da, mask_feature_da
    )
    xr.testing.assert_equal(result, expected)
...
def test_union_features_basic(base_dataset):
    ...
    union_mapping = {'feature_2d_hindcast': 'mask_2d'}
    # Set a specific value in mask_2d to fill the NaN in feature_2d_hindcast
    base_dataset['mask_2d'].loc[{'basin': 'basin_A', 'date': '2000-01-02'}] = (
        99.0
    )
    ...
    expected_feature_2d_hindcast_data = base_dataset[
        'feature_2d_hindcast'
    ].values.copy()
    ...
    xr.testing.assert_equal(
        result_ds['feature_2d_hindcast'], expected_feature_2d_hindcast
    )
...
def test_union_features_unmasked_features(base_dataset):
    ...
    assert (
        'mask_2d' in result_ds.data_vars
    )  # Masking features are also included
    assert 'mask_3d' in result_ds.data_vars
    assert (
        'feature_3d_forecast' in result_ds.data_vars
    )  # Not explicitly masked, so should be there
...
def test_union_features_self_union(base_dataset):
    ...
    union_mapping = {'feature_2d_hindcast': 'feature_2d_hindcast'}
    original_feature_2d = base_dataset['feature_2d_hindcast'].copy(deep=True)
    result_ds = union_features(base_dataset, union_mapping)
    xr.testing.assert_equal(
        result_ds['feature_2d_hindcast'], original_feature_2d
    )
...
def test_union_features_missing_feature_to_mask(base_dataset):
    ...
    """
    union_mapping = {'non_existent_feature': 'mask_2d'}
    result_ds = union_features(base_dataset, union_mapping)
...
def test_union_features_missing_mask_feature_raises_error(base_dataset):
    ...
    union_mapping = {'feature_2d_hindcast': 'non_existent_mask_feature'}
    with pytest.raises(
        ValueError,
        match='Masking feature non_existent_mask_feature not in dataset.',
    ):
...
def test_union_features_empty_mapping(base_dataset):
    ...
    result_ds = union_features(base_dataset, union_mapping)
    xr.testing.assert_equal(
        result_ds, base_dataset
    )  # Should be identical as nothing was masked
...
def test_union_features_all_features_masked(base_dataset):
    ...
    union_mapping = {
        f: f for f in all_features
    }  # Map each feature to itself for simplicity

    result_ds = union_features(base_dataset, union_mapping)
    xr.testing.assert_equal(
        result_ds, base_dataset
    )  # Should be identical if mapped to self
    ...
    xr.testing.assert_equal(
        result_ds_real['feature_2d_hindcast'], expected_feature_2d_hindcast
    )
    ...
    xr.testing.assert_equal(
        result_ds_real['feature_scalar'], base_dataset['feature_scalar']
    )
    ...
    xr.testing.assert_equal(
        result_ds_real['other_feature'], base_dataset['other_feature']
    )
    assert 'feature_3d_forecast' in result_ds_real
    xr.testing.assert_equal(
        result_ds_real['feature_3d_forecast'],
        base_dataset['feature_3d_forecast'],
    )
