...
def test_sort_frequencies():
    ...
    assert sort_frequencies(['1D', '1h', '2D', '3h']) == [
        '2D',
        '1D',
        '3h',
        '1h',
    ]
    assert sort_frequencies(['1' + _ME_FREQ, '1' + _YE_FREQ]) == [
        '1' + _YE_FREQ,
        '1' + _ME_FREQ,
    ]
    ...
    pytest.raises(
        ValueError, sort_frequencies, ['1D', '1XYZ']
    )  # not a frequency
...
def test_infer_frequency():
    ...
    assert (
        infer_frequency(pd.date_range('2000-01-01', '2000-01-10', freq='D'))
        == '1D'
    )
    assert (
        infer_frequency(pd.date_range('2000-01-01', '2000-01-10', freq='48h'))
        == '2D'
    )
    assert (
        infer_frequency(pd.date_range('2000-01-01', '2000-03-01', freq='W-MON'))
        == '7D'
    )
