    ...
    for var_name, data_array_data in data_vars.items():
        ...
        if lead_times is not None:
            ...
            data_array = xr.DataArray(
                np.array(data_array_data).reshape(
                    len(basins), len(dates), len(lead_times)
                ),
                coords=coords,
                dims=dims,
            )
...
def test_flatten_feature_groups_mixed_list_raises_error():
    ...
    expected = [
        'a',
        ['b', 'c'],
    ]  # It will not flatten if the first element is not a list
    with pytest.raises(
        ValueError,
        match='A mix of lists and features was supplied as feature groups.',
    ):
...
def test_flatten_feature_groups_non_list_input_raises_value_error():
    ...
    with pytest.raises(
        ValueError, match='Feature groups must be supplied as a list.'
    ):
...
def test_extract_feature_groups_partial_groups_raises_error():
    ...
    with pytest.raises(
        ValueError, match='There appear to be mixed groups in the dataset.'
    ):
        extract_feature_groups(groups, features)
...
def test_extract_feature_groups_missing_features_raises_error():
    ...
    with pytest.raises(
        ValueError, match='Not all features are in feature groups: '
    ):
        extract_feature_groups(groups, features)
...
def test_validate_samples_any_no_lead_time_some_nan(
    ...
    dataset = create_test_dataset(
        data_vars, basins_fixture, sample_dates_fixture
    )
...
def test_validate_samples_any_with_lead_time_some_nan(
    basins_fixture, sample_dates_fixture
):
    ...
    dataset = create_test_dataset(
        data_vars, basins_fixture, sample_dates_fixture, lead_times
    )
    mask = validate_samples_any(dataset)
    expected_mask_data = np.array(
        [
            ...
            [
                ...
                True,
                False,
            ],  # basin_B, date=2020-01-05 is False
            [True, True, True, True, True],
        ]
    )
    expected_mask = xr.DataArray(
...
def test_validate_samples_all_no_lead_time_some_nan(
    basins_fixture, sample_dates_fixture
):
...
def test_validate_samples_all_with_lead_time_some_nan(
    basins_fixture, sample_dates_fixture
):
    ...
    expected_mask_data = np.array(
        [
            [
                ...
                True,
                True,
            ],  # basin_A, date=2020-01-02 is False
            [
                True,
                True,
                True,
...
def test_validate_samples_any_all_group_success(
    basins_fixture, sample_dates_fixture
):
...
def test_validate_samples_any_all_group_no_valid_group(
    basins_fixture, sample_dates_fixture
):
    ...
    expected_mask_data = np.array(
        [
            [
                ...
                False,
                True,
            ],  # basin_A, date=2020-01-04 is False
            [True, True, True, True, True],
            [True, True, True, True, True],
        ]
    )
...
def test_validate_samples_any_all_group_empty_feature_groups(
    basins_fixture, sample_dates_fixture
):
...
def test_validate_samples_all_any_group_success(
    basins_fixture, sample_dates_fixture
):
...
def test_validate_samples_all_any_group_one_group_fails_any(
    basins_fixture, sample_dates_fixture
):
    ...
    dataset_fail = create_test_dataset(
        data_vars_fail, basins_fixture, sample_dates_fixture
    )
    ...
    expected_mask_data_fail = np.array(
        [
            [
                ...
                True,
                True,
            ],  # basin_A, date=2020-01-01 is False
            [True, True, True, True, True],
            [True, True, True, True, True],
        ]
    )
...
def test_validate_samples_all_any_group_empty_feature_groups(
    basins_fixture, sample_dates_fixture
):
...
@patch(
    'googlehydrology.datautils.validate_samples.validate_samples_any_all_group'
)
@patch(
    'googlehydrology.datautils.validate_samples.validate_samples_all_any_group'
)
def test_validate_samples_for_nan_handling_dispatch(
    ...
    mock_all,
    basins_fixture,
    sample_dates_fixture,
):
    """Test validate_samples_for_nan_handling dispatches to correct function."""
    dummy_dataset = create_test_dataset(
        {'var': np.ones((len(basins_fixture), len(sample_dates_fixture)))},
        basins_fixture,
        sample_dates_fixture,
    )
    dummy_feature_groups = [['var']]
    mock_return_mask = xr.DataArray(
        True,
        coords={'basin': basins_fixture[0], 'date': sample_dates_fixture[0]},
        dims=['basin', 'date'],
    )
    ...
    validate_samples_for_nan_handling(
        dummy_dataset, 'input_replacing', dummy_feature_groups
    )
    ...
    # Test 'masked_mean'
    validate_samples_for_nan_handling(
        dummy_dataset, 'masked_mean', dummy_feature_groups
    )
    mock_any_all_group.assert_called_once_with(
        dummy_dataset, dummy_feature_groups
    )
    mock_any_all_group.reset_mock()

    # Test 'attention'
    validate_samples_for_nan_handling(
        dummy_dataset, 'attention', dummy_feature_groups
    )
    mock_any_all_group.assert_called_once_with(
        dummy_dataset, dummy_feature_groups
    )
    ...
    # Test 'unioning'
    validate_samples_for_nan_handling(
        dummy_dataset, 'unioning', dummy_feature_groups
    )
    mock_all_any_group.assert_called_once_with(
        dummy_dataset, dummy_feature_groups
    )
    ...
    with pytest.raises(
        ValueError, match='Unrecognized NaN-handling method: unknown_method.'
    ):
        validate_samples_for_nan_handling(
            dummy_dataset, 'unknown_method', dummy_feature_groups
        )
...
def test_validate_sequence_all_basic(sample_dates_fixture):
    ...
    mask = xr.DataArray(
        mask_data, coords={'date': sample_dates_fixture}, dims=['date']
    )
    seq_length = 2
    shift_right = 0
    ...
    expected_mask = xr.DataArray(
        expected_data, coords={'date': sample_dates_fixture}, dims=['date']
    )
...
@patch('googlehydrology.datautils.validate_samples.validate_samples_all')
@patch(
    'googlehydrology.datautils.validate_samples.validate_samples_for_nan_handling'
)
...
def test_validate_samples_no_features_raises_error(
    ...
    with pytest.raises(
        ValueError,
        match='At least one feature list is required to validate samples.',
    ):
        validate_samples(
            is_train=False,
...
def test_validate_samples_static_features_only(
    ...
    dummy_dataset = create_test_dataset(
        {
            'static_var': np.ones(
                (len(basins_fixture), len(sample_dates_fixture))
            )
        },
        basins_fixture,
        sample_dates_fixture,
    ...
    mock_validate_samples_all.assert_called_once_with(
        dataset=dummy_dataset[['static_var']]
    )
...
def test_validate_samples_hindcast_features(
    ...
    dummy_dataset = create_test_dataset(
        {
            'h1': np.ones((len(basins_fixture), len(sample_dates_fixture))),
...
def test_validate_samples_forecast_features(
    mock_validate_samples_for_nan_handling, basins_fixture, sample_dates_fixture
):
...
def test_validate_samples_forecast_features_with_overlap(
    ...
    mock_validate_samples_for_nan_handling.side_effect = [
        mock_mask,
        mock_mask,
    ]  # For forecast and forecast_overlap calls
    mock_validate_sequence_all.return_value = mock_mask
    ...
    assert (
        mock_validate_samples_for_nan_handling.call_count == 2
    )  # Once for forecast, once for forecast_overlap
    assert (
        mock_validate_sequence_all.call_count == 1
    )  # Once for forecast_overlap
    assert len(masks) == 3
    assert masks[0].name == 'forecasts'
...
def test_validate_samples_target_features_train(
    ...
    mock_validate_samples_any.assert_called_once_with(
        dataset=dummy_dataset[['t1']]
    )
...
def test_validate_samples_target_features_train_zeros(
    mock_validate_sequence_any, basins_fixture, sample_dates_fixture
):
...
def test_validate_samples_hindcast_missing_seq_length_raises_error(
    ...
    with pytest.raises(
        ValueError,
        match='Sequence length is required when validating hindcast data.',
    ):
        validate_samples(
            is_train=False,
            dataset=dummy_dataset,
...
def test_validate_samples_forecast_overlap_missing_min_lead_time_raises_error(
    ...
    with pytest.raises(
        ValueError,
        match='`min_lead_time`is required when validating a forecast overlap sequence.',
    ):
        validate_samples(
            is_train=False,
            dataset=dummy_dataset,
...
def test_validate_samples_target_missing_predict_last_n_raises_error(
    ...
    with pytest.raises(
        ValueError,
        match='Target sequence length is required when validating target data.',
    ):
        validate_samples(
