...
def _flatten_feature_groups(groups: list[list[str]]) -> list[str]:
    ...
    if any(isinstance(g, list) for g in groups) and not all(
        isinstance(g, list) for g in groups
    ):
        raise ValueError(
            'A mix of lists and features was supplied as feature groups.'
        )
...
def extract_feature_groups(
    groups: list[list[str]], features: list[str]
) -> list[list[str]]:
    ...
    if missing_features:
        raise ValueError(
            f'Not all features are in feature groups: {missing_features}.'
        )
