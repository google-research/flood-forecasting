...
from googlehydrology.evaluation.utils import metrics_to_dataframe
from googlehydrology.utils.config import Config
from googlehydrology.utils.errors import AllNaNError


def create_results_ensemble(
    run_dirs: list[Path],
    best_k: int = None,
...
def _create_ensemble(
    results_files: list[Path], frequencies: list[str], config: Config
) -> dict:
            ...
            for freq in frequencies:
                ...
                if freq not in ensemble_sum[basin]:
                    ...
                    for target_var in target_vars:
                        ensemble_sum[basin][freq][f'{target_var}_sim'] += (
                            freq_results[f'{target_var}_sim']
                        )
    ...
    for basin in tqdm(ensemble_sum.keys()):
        for freq in frequencies:
            ...
            freq_date_range = freq_date_range[
                np.tile(mask, len(ensemble_xr.coords['date']))
            ]

            ensemble_xr = (
                ensemble_xr.isel(time_step=slice(-frequency_factor, None))
            ...
            for target_var in target_vars:
                ...
                metrics = (
                    ...
                    else config.metrics[target_var]
                )
                if 'all' in metrics:
...
def _get_best_validation_runs(
    run_dirs: list[Path], k: int, epoch: int = None
) -> list[Path]:
    ...
    val_files = list(
        zip(
            run_dirs,
            [
                _get_results_file(run_dir, 'validation', epoch)
...
def _get_results_file(
    run_dir: Path, period: str = 'test', epoch: int = None
) -> Path:
    """Returns the path of the results file in the given run directory."""
    if epoch is not None:
        dir_results_files = list(
            Path(run_dir).glob(
                f'{period}/model_epoch{str(epoch).zfill(3)}/{period}_results.p'
            )
        )
    else:
        dir_results_files = list(
            Path(run_dir).glob(f'{period}/model_epoch*/{period}_results.p')
        )
...
def _main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        help='Directories of the runs to be averaged.',
    )
    parser.add_argument(
        '--period',
        type=str,
    ...
    try:
        df = metrics_to_dataframe(
            ensemble_results, metrics, cfg.target_variables
        )
