...
def _warn_missing_variables_once(
  ...
  if not missing_bands:
    return
  key = (store_uri, product.value, tuple(sorted(missing_bands)))
  ...
  logger.warning(
      ...
      "Those band(s) will be populated with NaN (missing data in -> missing "
      "data out).",
      store_uri,
      product.value,
      list(missing_bands),
...
def extract_nowcast_from_archive(
  ...
  source_vars = sorted({src for _, src in resolved_bands.values()})
  def _compute_sub():
    with dask.config.set(scheduler="threads"):
      return sub[source_vars].compute()

  sub_loaded = _retry_with_backoff(
      _compute_sub,
      description=f"reading {spec.product.value} slices from {uri}",
      max_retries=max_retries,
  )

  sub_dates = pd.DatetimeIndex(
      pd.to_datetime(sub_loaded["time"].values).floor("D")
  )
  valid_sub_indices = []
  ...
  for target_band, (band_spec, src_name) in resolved_bands.items():
    raw_vals = np.asarray(sub_loaded[src_name].values, dtype=np.float32)
    ...
    if valid_target_indices:
      ...
      if not coverage_recorded:
        missing_fraction_out[:, valid_target_indices] = reduced_missing[
            :, valid_sub_indices
        ]
...
def extract_forecast_from_archive(
  ...
  sub_loaded = _retry_with_backoff(
      _compute_sub,
      description=f"reading {spec.product.value} forecast slices from {uri}",
      max_retries=max_retries,
  )
  ...
  # Align archive lead_time axis (1..10 days, either int or timedelta64)
  raw_leads = sub_loaded["lead_time"].values
  ...
  for target_band, (band_spec, src_name) in resolved_bands.items():
    da = sub_loaded[src_name].transpose(
        "time", "lead_time", "latitude", "longitude"
    )
    raw_vals = np.asarray(da.values, dtype=np.float32)
